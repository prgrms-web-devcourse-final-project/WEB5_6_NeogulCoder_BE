...
public interface StudyPostCommentSpecification {
    ...
    ApiResponse<Long> create(@Parameter(description = "게시글 ID", example = "10") long postId, StudyPostCommentSaveRequest request, Principal userDetails);

    @Operation(summary = "댓글 수정", description = "댓글을 수정 합니다.")
    ApiResponse<Void> update(long commentId, StudyCommentUpdateRequest request, Principal userDetails);

    @Operation(summary = "댓글 삭제", description = "댓글을 삭제합니다.")
    ApiResponse<Void> delete(@Parameter(description = "댓글 ID", example = "100") long commentId,
